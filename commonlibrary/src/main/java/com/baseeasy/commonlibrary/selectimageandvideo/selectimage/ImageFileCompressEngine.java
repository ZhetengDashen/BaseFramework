...
public class ImageFileCompressEngine implements CompressFileEngine {
    ...
    public void onStartCompress(Context context, ArrayList<Uri> source, OnKeyValueResultCallbackListener call) {
        Luban.with(context).load(source).ignoreBy(300).setTargetDir(FileUtils.SDPATH+ PictureShared.FolderNameConfig.COMPRESSION).setRenameListener(new OnRenameListener() {
            @Override
